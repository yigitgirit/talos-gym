...
import { ClubResponse, ClubUpdateRequest, ClubUpdateRequestSchema } from '@/lib/api/schema/club.schema'
import { updateClubAction } from '@/features/clubs/actions/club.actions'
...
const LOCATION_PROVIDERS = ['GOOGLE_PLACES', 'MAPBOX', 'LOCATION_IQ', 'OSM', 'OTHER']
...
export function EditClubForm({ club, onCancelAction }: EditClubFormProps) {
    const router = useRouter()

    // Initialize form with existing club data
    const form = useForm<ClubUpdateRequest>({
        resolver: zodResolver(ClubUpdateRequestSchema),
        mode: 'onBlur',
        defaultValues: {
            name: club.name,
            slug: club.slug,
            address: {
                provider: club.address.provider || 'LOCATION_IQ',
                externalLocationId: club.address.externalLocationId || '',
            },
            timeZone: club.timeZone,
            description: club.description || undefined,
            photoUrls: club.photoUrls || [],
            scoreMultiplier: club.scoreMultiplier,
            active: club.active,
        },
    })
    ...
    const { execute, isPending } = useServerAction(updateClubAction, {
        onSuccess: () => {
            ...
            router.refresh() // Ensures changes reflect on the server immediately
        },
        onError: (error) => handleFormServerErrors(error, form.setError),
    })

    const onSubmit = form.handleSubmit((data) => {
        execute({ id: club.id.toString(), data })
    })
    ...
    return (
        <Card>
            ...
            <CardContent>
                ...
                <form id="edit-club-form" onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Controller
                        name="name"
                        control={form.control}
            ...
            <CardFooter className="flex justify-end gap-3">
                <Button variant="outline" onClick={() => onCancelAction ? onCancelAction() : router.push('/dashboard/clubs')} disabled={isPending}>
                    Cancel
                </Button>
                <Button type="submit" form="edit-club-form" disabled={isPending}>
